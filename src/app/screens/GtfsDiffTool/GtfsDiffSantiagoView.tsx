...
import santiagoBreakingChanges from './santiago-breaking-changes.json';

export default function GtfsDiffSantiagoView(): React.ReactElement {
  const diff = santiagoDiff as unknown as GtfsDiff;
  const newReport = santiagoValidationReportNew as ValidationReport;
  const oldReport = santiagoValidationReportOld as ValidationReport;
  const breakingChanges = santiagoBreakingChanges as unknown as BreakingChangeReport;
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', py: 4, px: 2 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1, flexWrap: 'wrap' }}>
        ...
        <Chip label='Metro de Santiago' size='small' color='primary' variant='outlined' />
      </Box>
      <Typography color='text.secondary' sx={{ mb: 1 }}>
        Diff report for Metro de Santiago / Red Metropolitana. Upload your own feeds on the{' '}
